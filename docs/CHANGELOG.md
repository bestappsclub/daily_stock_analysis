...
- [新功能] Web「选股」页新增「同步行情缓存」按钮（美股/新加坡）：接口 `POST /api/v1/alphasift/sync-cache` 触发当前市场增量同步本地行情缓存，与 CLI `scripts/sync_prices.py`、每日定时任务共用 `MarketScreenerService.sync_cache`。
