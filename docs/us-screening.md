...
| `us_rs_leaders` | 相对强弱领涨 | **跑赢大盘且 RS 线上行**（leading）的多头标的，按相对大盘超额收益排序 |
| `us_trend_confirmed` | ADX趋势确认 | **多头趋势 + ADX≥阈值**（趋势强、非震荡），按 ADX 排序的高质量趋势股 |
...
> 出场/止损位（与 stockscreener 一致）：每个候选给出 **吊灯止损 Chandelier Exit**（`chandelier_stop` = `HHV/LLV(close, CHANDELIER_LEN=22) − CHANDELIER_MULT=3×ATR`，棘轮只进不退；`chandelier_dir` 1/-1）与 **DK 持仓 12% 移动止损**（`dk_trail_stop` = 持仓期峰值收盘 ×(1−`DK_TRAIL_PCT=0.12`)，仅持股态有值）。候选理由标注如「止损：吊灯多 297.36 / 移止12% 277.38」。
...
> DK 买卖点（东财校准版）：**收盘价唐奇安通道**状态机——收盘上破 `HHV(close, n_up)`（或上破短周期 `HHV(close, n_soft)` 且放量）转**持股(D点/买)**，跌破 `LLV(close, n_dn)` 转**持币(K点/卖)**；中间形成滞后带，信号稀疏。参数 `DK_NUP=20 / DK_NSOFT=7 / DK_NDN=10 / DK_VWIN=20 / DK_VMULT=1.0`（可经环境变量覆盖），与 stockscreener 项目 `technical.py:_dk_buysell_state`（按东财提示逆向标定）一致；个股结果含 `dk_state`/`dk_signal`/`dk_last_signal`/`dk_days_since`。需前复权数据对齐东财。完整算法见 stockscreener 项目 `docs/dk-indicator.md`。

> ADX/DMI（趋势强度，`trend_confirmed` 策略 + 个股 `adx`/`plus_di`/`minus_di`/`adx_status` 字段）：Wilder 平滑（`ADX_LEN=14`，与 RSI 同口径 `ewm(alpha=1/N)`）算出 TR/+DM/−DM → +DI/−DI → DX → ADX。`adx_status`：`adx≥ADX_STRONG(40)` = strong_trend，`≥ADX_TREND_MIN(25)` = trend，否则 range（震荡）。ADX 不给买卖方向，只判断"现在是否有趋势"——震荡市里趋势/突破类策略易失效，故 `trend_confirmed` 用它过滤出"多头 + ADX 确认"的标的。

> OBV 量价确认/背离（个股 `obv`/`obv_trend`/`obv_divergence`/`vol_confirm_desc` 字段）：收盘上涨累加当日量、下跌减去当日量。在 `OBV_DIV_WIN=20` 窗口内比较价与 OBV 同期变化方向——价涨而 OBV 走弱 = **顶背离（bearish）**（上涨缺量能支撑，提防假突破，会追加一条 `risk_factors`）；价跌而 OBV 走强 = **底背离（bullish）**；方向一致 = 量价配合。用于给突破/趋势信号做量能确认，不单列策略。

> 相对强弱 RS（个股 vs 大盘指数，`rs_leaders` 策略 + 个股 `rs_ratio`/`rs_chg_pct`/`rs_status` 字段）：按交易日对齐大盘指数，取 `RS_LOOKBACK=60` 窗口比较累计收益——`rs_chg_pct=(个股收益−大盘收益)×100`（正=跑赢），`rs_ratio=(1+个股)/(1+大盘)`（>1=跑赢），并看 RS 线（个股/大盘）是否上行。`rs_status`：跑赢且 RS 上行=leading，跑输且 RS 下行=lagging，否则 neutral。基准指数按市场取 `^GSPC`/`^STI`/`^HSI`/沪深300（`<PREFIX>_BENCHMARK` 可覆盖）；**指数抓取失败则 RS 中性、fail-open，不影响选股**。动量是少数长期稳定有效的因子，`rs_leaders` 即"只买跑赢大盘的多头股"。
...
| `<PREFIX>_BENCHMARK` | `^GSPC` | `^STI` | 相对强弱 RS 的大盘基准指数（hk `^HSI` / cn 沪深300 `000300`）；留空禁用 RS，抓取失败则 RS 中性 |

> 三指标全局参数（非市场前缀，见 `.env.example`）：`ADX_LEN`/`ADX_TREND_MIN`/`ADX_STRONG`（ADX）、`OBV_DIV_WIN`（OBV 背离窗口）、`RS_LOOKBACK`（RS 回看窗口）。均有默认值，不配置也可运行。
...
- 因子与评分复用统一的趋势引擎（均线/量价/MACD/RSI/ADX/OBV/相对强弱 RS），未做各市场专属因子定制。
- 相对强弱 RS 依赖外部大盘指数（yfinance/akshare）：指数源限流或代码变动时该指标降级为中性，不影响其余分析与排序。
